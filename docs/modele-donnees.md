...
7. Une recette appartient à une catégorie. Elle possède zéro ou un auteur, pour permettre une recette externe ou conserver une recette après suppression de son auteur. Les administrateurs gèrent les recettes sans auteur ; l’API refuse leur modification et suppression aux autres comptes.
8. Une recette exploitable possède au moins un ingrédient. Le couple recette/ingrédient est unique ; la quantité concerne le nombre de portions indiqué dans Recipe.servings. La création complète et le remplacement d’une recette sont transactionnels. Le service impose entre 1 et 100 ingrédients distincts : une simple clé étrangère ne garantit pas ce minimum.
...
Le CRUD du stock et la recherche du catalogue sont maintenant exposés par l’API : [contrat](api-stock.md), [fiche de compréhension](Comprendre_le_stock.md) et [29 tests HTTP réussis](verification-stock-2026-09-20.txt). Les contraintes du modèle n’ont pas changé. Le contrôle de propriété, les conversions et le refus des doublons sont vérifiés. La création des recettes est décrite dans l’évolution ci-dessous. Les suggestions restent à implémenter.

## Évolution des recettes le 20 septembre 2026

La lecture et la recherche des recettes sont publiques. L’API réserve les écritures à l’auteur et aux administrateurs. La création et le remplacement de la composition utilisent des transactions SERIALIZABLE. Le remplacement retire les anciennes associations de préférences, à réévaluer dans l’étape dédiée. Le modèle physique n’a pas changé. Voir le [contrat](api-recettes.md), la [fiche de compréhension](Comprendre_les_recettes.md) et les [30 tests HTTP réussis](verification-recettes-2026-09-20.txt), dont annulation sur panne SQL et écritures concurrentes.
